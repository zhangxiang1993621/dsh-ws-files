    ...
    function WsFilesPanel() {
      ...
      const renderNode = (node, path, depth) => {
        ...
        return h('div', { key: path },
          h('div', {
            style: { ...S.row, paddingLeft: 8 + depth * 16, fontWeight: isDir ? 600 : 400 },
            onClick: () => toggle(node, path),
            title: path,
          }, icon + '  ' + node.name + (isDir ? '' : '  ' + fmtSize(node.size))),
          isDir && st && st.loading
            ? h('div', { style: { ...S.hint, paddingLeft: 26 + depth * 16 } }, '加载中…')
            : null,
          isDir && st && st.error
            ? h('div', { style: { ...S.err, paddingLeft: 26 + depth * 16 } }, '⚠ ' + st.error)
            : null,
          isDir && expanded && st.children
            ? h('div', null, st.children.map(c => renderNode(c, path ? path + '/' + c.name : c.name, depth + 1)))
            : null,
        )
      }
