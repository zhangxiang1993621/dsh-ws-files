 ...
 * 标签（标签条上直接有 × 可关闭）；改动自动保存（防抖），Ctrl+Z/Ctrl+Y 撤销/重做，Ctrl+S 立即保存。
 * 预览模式保留行号、按文件类型做关键词/注释/字符串高亮与 tab-size，并按缩进层级折叠代码块。
 */
  ...
  factory: (require) => {
    ...
    const openFiles = {
      ...
      open(root, path, name) {
        ...
        this.files[id] = {
          id, root, path, name, content: null, loading: true, error: null,
      ...
      async save(id) {
        ...
        const toSave = f.draft
        if (toSave === f.content) return
        f.saving = true
        f.saveError = null
        this.emit()
        try {
